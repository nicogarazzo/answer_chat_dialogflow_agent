...
const GPROJECTID='doctor-appointment-b9og'
const TARGETQUEUE='from-dialogflow'
const HANDOFFINTENT='to-operator'
// kit.incomingMessage.current_request.id
async function runSample(projectId = GPROJECTID, text, session_id) {
...
  if (session_id == null) result = await runSample(GPROJECTID,  kit.incomingMessage.text, true)
  else result = await runSample(GPROJECTID,  kit.incomingMessage.text, session_id)
  //console.log(result)
  if (result.diagnosticInfo && result.diagnosticInfo.fields && result.diagnosticInfo.fields.end_conversation) {
      console.log('FINISH!')
      if (result.intent.displayName == HANDOFFINTENT) {
          console.log('transferToQueue')
          kit.transferToQueue({ queue_name: TARGETQUEUE })
